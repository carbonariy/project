...
SMSC_POST = getattr(settings, 'SMSC_POST', True)  # использовать метод POST
SMSC_HTTPS = getattr(settings, 'SMSC_HTTPS', True)  # использовать HTTPS протокол
SMSC_CHARSET = getattr(settings, 'SMSC_CHARSET', 'utf-8')  # кодировка сообщения
SMSC_DEBUG = getattr(settings, 'SMSC_DEBUG', False)  # флаг отладки
...
class SMSC(object):
    ...
    def send_sms(self, phones, message, translit=0, time="", id=0, format=0, sender=False, query=""):
        ...
        if SMSC_DEBUG:
            if m[1] > "0":
                LOGGER.debug("Сообщение отправлено успешно. ID: " + m[0] + ", всего SMS: " + m[1] + ", стоимость: " + m[
                    2] + ", баланс: " + m[3])
            else:
                LOGGER.debug("Ошибка №" + m[1][1:] + ifs(m[0] > "0", ", ID: " + m[0], ""))
    ...
    def _smsc_send_cmd(self, cmd, arg=""):
        ...
        if ret == "":
            if SMSC_DEBUG:
                LOGGER.debug("Ошибка чтения адреса: " + url)
            ret = ","  # фиктивный ответ
